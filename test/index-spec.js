...
describe('intermediate', function () {
  ...
  it('copies files to the OS temp directory', function (done) {
    var testProcess = (tempDir, cb) => {
      var testPaths = _.pluck(this.testFiles, 'path');
      var tempPaths = [];
    ...
    stream.write(this.testFiles[0]);
    stream.write(this.testFiles[1]);
    stream.write(this.testFiles[2]);
    stream.resume();
    stream.end();
  });
  ...
  it('exposes vinyl files to the process callback', function (done) {
    var testProcess = (tempDir, cb, vinyl) => {
      assert.deepEqual(vinyl, this.testFiles);
      cb();
    };
